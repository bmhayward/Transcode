...
Transcode uses the `transcode-video tool` to convert audio and video. By default, Transcode uses the `transcode-video --quick` quality option. This setting can be modified in Settings>Output quality. Entering comma separated values into the Output quality field, allows the transcoding of video with multiple quality settings.
